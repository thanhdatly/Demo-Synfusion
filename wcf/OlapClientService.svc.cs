...
namespace SyncfusionMvcApplication3
{
    ...
    public class OlapClientService : IOlapClientService
    {
        ...
        string conStringforDB = "DataSource=" + HttpContext.Current.Server.MapPath(".").Split(new string[] { "\\wcf" }, StringSplitOptions.None)[0] + "\\App_Data\\ReportsTable.sdf; Persist Security Info=False";
        ...
        public Dictionary<string, object> CubeChanged(string action, string cubeName, string olapReport, string clientReports, string clientParams)
        {
            OlapDataManager DataManager = new OlapDataManager(connectionString);
            if (olapReport != "")
                DataManager.SetCurrentReport(OLAPUTILS.Utils.DeserializeOlapReport(olapReport));
